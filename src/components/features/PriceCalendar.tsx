...
import { ChevronLeft, ChevronRight, TrendingDown, Sparkles, Calendar, Users, Plane } from 'lucide-react';
...
const PRICE_STYLES: Record<PriceLevel, { bg: string; text: string; border: string; hover: string; dot: string }> = {
  lowest: {
...
export function PriceCalendar({
  ...
  return (
    <Card 
      ...
      role="application"
      aria-label="Price calendar - Find the cheapest dates to fly"
    >
      {/* Header */}
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-white/70 text-xs">Find the best deals</p>
              </div>
            </div>
            ...
            {priceCount > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 backdrop-blur rounded-full">
                ...
                <span className="text-white text-xs font-medium">{priceCount} flights</span>
              </div>
            )}
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {lowestDate && (
                <span className="text-sm text-emerald-600 dark:text-emerald-400">
                  Best price on <strong>{format(lowestDate, 'MMM d')}</strong>
                </span>
              )}
            </div>
      ...
      <div className="p-4 sm:p-5">
          ...
          {days.map(day => {
            ...
            return (
              <button
                key={dateKey}
                data-date={dateKey}
                onClick={() => !isPast && price && onSelectDate?.(day)}
                ...
                aria-label={`${format(day, 'EEEE, MMMM d')}: ${price ? `${formatPrice(price, currency)}${isLowestDay ? ' - Best price!' : ''}` : 'No flights'}`}
                ...
                className={cn(
                  ...
                  // Selected state
                  isSelected && 'ring-2 ring-emerald-500 ring-offset-2 dark:ring-offset-neutral-900 scale-105 shadow-lg z-10',
                  
                  // Hovered state
                  (isHovered || isFocused) && price && !isPast && 'shadow-lg scale-105 z-10',
                  
                  // Today indicator
                  isTodayDate && !isSelected && 'ring-2 ring-blue-400 dark:ring-blue-500 ring-offset-1',
        ...
        <p className="sr-only">
          Use arrow keys to navigate dates. Page Up and Page Down to change months. Enter to select a date.
        </p>
      </div>
    </Card>
  );
